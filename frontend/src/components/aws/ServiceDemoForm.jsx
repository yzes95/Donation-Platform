...
const PRESETS = [
  {
    ...
    category: 'seasonal',
    category_label: 'موسمي / Seasonal',
    target_amount: 3500,
    family_id: 'fam-02',
    family_name: 'أسرة عم إبراهيم أبو نافع',
  ...
  },
  {
    ...
    category: 'housing',
    category_label: 'سكن وعلاج / Housing & Health',
    target_amount: 8500,
    family_id: 'fam-01',
    family_name: 'أسرة عمار أبو شويعي',
  ...
  },
  {
    ...
    category_label: 'سداد ديون / Debt Relief',
    target_amount: 4200,
    family_id: 'fam-02',
    family_name: 'أسرة عم إبراهيم أبو نافع',
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2.5 flex-1 justify-end">
          ...
          <div className="flex items-center gap-1.5 bg-gray-800/80 border border-gray-700/80 rounded-xl px-2.5 py-1.5">
            <span className="text-xs text-gray-400 shrink-0">الحالة / Case:</span>
            <select
              disabled={isBusy}
              value={selectedPreset.id}
              onChange={(e) => handlePresetChange(e.target.value)}
              className="bg-transparent text-xs text-emerald-300 font-medium focus:outline-none cursor-pointer max-w-[210px] truncate"
            >
              {PRESETS.map(p => (
          ...
          <div className="flex items-center gap-1 bg-gray-800/80 border border-gray-700/80 rounded-xl px-2.5 py-1.5">
            <span className="text-xs text-gray-400 shrink-0">المبلغ:</span>
            <input
              type="number"
              disabled={isBusy}
              value={amount}
            ...
            />
            <span className="text-[11px] text-gray-400">ج.م</span>
          </div>
          ...
          >
            ...
            <span className="hidden lg:inline">{showDetails ? 'إخفاء' : 'تفاصيل'}</span>
          </button>
          ...
          {simulationStep === 'complete' ? (
            ...
            >
              ...
              <span>{isBusy ? 'جاري التنفيذ...' : 'Send to AWS Backend'}</span>
            </button>
          )}
        </div>
      </div>
      ...
      {showDetails && (
        <div className="pt-2 border-t border-gray-800/80 grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
          <div className="bg-gray-950/60 border border-gray-800/70 p-2.5 rounded-xl">
            <span className="text-gray-500 block mb-0.5">الأسرة المستفيدة:</span>
            <span className="text-gray-200 font-medium">{selectedPreset.family_name}</span>
          </div>
          <div className="bg-gray-950/60 border border-gray-800/70 p-2.5 rounded-xl">
            <span className="text-gray-500 block mb-0.5">التصنيف والهدف:</span>
            <span className="text-gray-200 font-medium">{selectedPreset.category_label}</span>
          </div>
          <div className="bg-gray-950/60 border border-gray-800/70 p-2.5 rounded-xl">
            <span className="text-gray-500 block mb-0.5">الوصف:</span>
            <span className="text-gray-300 leading-relaxed" dir="rtl">{selectedPreset.description_ar}</span>
          </div>
        </div>
      )}
        ...
        <div className="pt-2 border-t border-gray-800/80 flex flex-wrap items-center justify-between gap-2">
          ...
          {simulationStep === 'complete' && (
            <div className="flex items-center gap-1.5 text-xs text-emerald-400 font-medium">
              ...
              <span>Created ID: <code className="font-mono text-emerald-300 font-bold">{refId}</code></span>
            </div>
          )}
          ...
          {simulationStep === 'error' && (
            <div className="flex items-center gap-1.5 text-xs text-red-400 font-medium">
              <AlertCircle size={14} />
              <span>Backend unreachable — check VITE_BACKEND_URL</span>
            </div>
          )}
        </div>
      )}
